...
function FormEditor() {
  ...
  const [name, setName] = useState(name_text);
  const [nickname, setNick] = useState(nick_text);
  const [department, setDepartment] = useState(department_text);
  const [phone, setPhone] = useState(phone_number);
  const [room, setRoom] = useState(room_number);
  const [building, setBuilding] = useState(building_number);
            ...
            <div className="bg-white rounded shadow-sm p-4 px-4 md:p-8 mb-6">
              <div className="grid gap-4 gap-y-2 text-sm grid-cols-1 lg:grid-cols-3">
                <div className="text-gray-800">
                  ...
                  <p>Please Refill out all the fields.</p>
                </div>
